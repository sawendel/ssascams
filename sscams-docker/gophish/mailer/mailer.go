...
func (mw *MailWorker) Start(ctx context.Context) {
	for {
		...
		case <-ctx.Done():
			return
		case ms := <-mw.queue:
			fmt.Println("imprimiendo el queue")
			fmt.Println(ms)
			if len(ms) > 0 {
...
func isDuplicated(fromAddress string, toAddress string, subject string) bool {
	...
	for rows.Next() {
		...
		if err2 != nil {
			fmt.Println("query row dio error")
			db.Close()
			duplicated = 0
		} else {
			db.Close()
...
func sendMail(ctx context.Context, dialer Dialer, ms []Mail) {
	...
	for i, m := range ms {
		...
		fromAddress := message.GetHeader("Sender")
		if len(fromAddress) == 0 {
			fromAddress = message.GetHeader("From")
		}

		if !(isDuplicated(fromAddress[0], message.GetHeader("To")[0], message.GetHeader("Subject")[0])) {
...
// func main() {
// 	// meter aquí params para probar
// 	isDuplicated("rodrigo@behavior.com", "nahomy@behavior.com")
// }
