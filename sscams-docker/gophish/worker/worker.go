...
func (w *DefaultWorker) LaunchCampaign(c models.Campaign) {
	log.Info("Entre al launch")
	ms, err := models.GetMailLogsByCampaign(c.Id)
	if err != nil {
		fmt.Println("no tengo los logs")
		log.Error(err)
		return
	}

	fmt.Println("pase el launch")
	...
	fmt.Println(campaignMailCtx)
	fmt.Println("encontre context")
	for _, m := range ms {
		fmt.Println(m.CampaignId)
	...
	fmt.Println("Imprimiendo los entries")
	fmt.Println(mailEntries)
