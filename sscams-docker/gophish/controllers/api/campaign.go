...
//struct for the received group name for relaunching a campaign
...
func (as *Server) Campaign(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == "POST":
		...
		var groupName models.Group
		groupName.Id = 0
		groupName.Name = group.Name
		groupName.ModifiedDate = time.Now().UTC()
		groupName.Targets = nil
		groupName.UserId = 0
		c.Groups = append(c.Groups, groupName)

		//err = models.PostCampaign(&c, ctx.Get(r, "user_id").(int64))
		err = models.RelaunchCampaign(&c, ctx.Get(r, "user_id").(int64))
		if err != nil {
			JSONResponse(w, models.Response{Success: false, Message: err.Error()}, http.StatusBadRequest)
			return
		}
		if c.Status == models.CampaignInProgress {
			as.worker.LaunchCampaign(c)
		}
		JSONResponse(w, "Campaign Relaunched Successfully", http.StatusOK)
	}
