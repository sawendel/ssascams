...
//to see if we have duplicates to avoid sending twice
func isDuplicated(fromAddress string, toAddress string, subject string) bool {
	h := fnv.New32a()
...
func saveGroupPerCampaign(campaignID int64, campaignName string, groupID int64, groupName string) {
	db, err := sql.Open("mysql", "testaws:pass1234@tcp(iredmail:3306)/vmail")
	if err != nil {
		panic(err)
	}
	...
	stmtIns, err := db.Prepare("INSERT INTO groups_per_campaign (campaignid, campaignname, groupid, groupname) VALUES (?,?,?,?)")
	if err != nil {
		panic(err.Error()) // proper error handling instead of panic in your app
	}
	defer stmtIns.Close()

	_, err = stmtIns.Exec(campaignID, campaignName, groupID, groupName)
	if err != nil {
		panic(err.Error()) // proper error handling instead of panic in your app
	}
}
...
func PostCampaign(c *Campaign, uid int64) error {
	...
	for _, g := range c.Groups {
		...
		saveGroupPerCampaign(c.Id, c.Name, g.Id, g.Name)
		for _, t := range g.Targets {
			if isDuplicated(c.SMTP.Username, t.Email, c.Template.Subject) {
				continue
			}
...
func RelaunchCampaign(c *Campaign, uid int64) error {
	err := c.Validate()
	if err != nil {
		return err
	}
	...
	for _, g := range c.Groups {
		...
		for _, t := range g.Targets {
			...
			// 	continue
			// }
			if CheckDuplicates(c.SMTP.Username, t.Email, c.Template.Subject) {
				continue
			}
			// Remove duplicate results - we should only
			// send emails to unique email addresses.
...
func CompleteCampaign(id int64, uid int64) error {
	...
	// TO-DO , do not mark the campaign as completed to keep sending mails under the campaign
	// lines commented with ## rnunez

	// Don't overwrite original completed time
	// ## rnunez if c.Status == CampaignComplete {
	// ## rnunez 	return nil
	// ## rnunez }
	// Mark the campaign as complete
	c.CompletedDate = time.Now().UTC()
	// ## rnunez c.Status = CampaignComplete
